...
# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 5 — FEEDBACK MODEL (full width)
# ══════════════════════════════════════════════════════════════════════════════
sl = add_slide()
header(sl, "HOW THE FEEDBACK MODEL WORKS")

# Flow subtitle
txt(sl, 0.5, 0.98, 12.3, 0.38,
    "Every correction a scientist makes becomes a rule that improves every future plan.",
    size=14, italic=True, colour=LIGHT_GREY, align=PP_ALIGN.CENTER)

# Three bucket cards — full width, stacked
bucket_data = [
    (EMERALD, "🏢  ORGANISATION SCOPE",
     "Applies to every plan generated by your organisation.\n"
     "Broad best practices seeded from experience — e.g. 'Always verify matrix effects for biosensors before ordering reagents.'"),
    (ACCENT,  "🔬  CATEGORY SCOPE",
     "Applies to a specific experiment type (Cell Biology, Diagnostics, Climate, …).\n"
     "Medium-scope patterns — e.g. 'For Cell Biology experiments: always include mycoplasma testing in the validation design.'"),
    (YELLOW,  "🧪  EXPERIMENT SCOPE",
     "Applies only when branching from a specific saved plan.\n"
     "Narrow, lab-specific corrections — e.g. 'n=4 replicates is sufficient for this HeLa cell line based on our historical variance data.'"),
]
for i, (col, label, desc) in enumerate(bucket_data):
    by = 1.48 + i * 1.55
    rect(sl, 0.4, by, 12.53, 1.38, RGBColor(0x0A, 0x1E, 0x30))
    rect(sl, 0.4, by, 12.53, 0.08, col)
    txt(sl, 0.62, by+0.13, 3.8,  0.38, label, size=14, bold=True, colour=col)
    txt(sl, 0.62, by+0.58, 11.9, 0.68, desc,  size=13, colour=LIGHT_GREY)

# Tech pipeline bar
rect(sl, 0.4, 6.17, 12.53, 0.72, RGBColor(0x05, 0x10, 0x1A))
rect(sl, 0.4, 6.17, 12.53, 0.06, RGBColor(0x33, 0x55, 0x77))
txt(sl, 0.60, 6.27, 12.1, 0.52,
    "Under the hood:  scientist correction  →  AI classifies bucket  →  "
    "Gemini 768-dim embedding  →  Supabase pgvector (HNSW index)  →  "
    "match_experiments() cosine search  →  injected as labelled block into next plan  ·  ~1,600 rules pre-seeded",
    size=11, colour=RGBColor(0x88, 0xBB, 0xDD))
